---
title: "Boosted regressions for examining the effects of climate change on species distributions"
...
```{r eval=FALSE}
#upload rasters of current climate variables
gl.current.list=list.files(path="pathways to files",
...
plot(predict_current, main="A. torren, current", 
     xlim=c(-95,-70), ylim=c(40,52))
```

```{r eval=FALSE}
#bring in rasters for future climate change projections
#use future projection RCP 45 2050 (compare climate at gps points to the great lakes region for the year 2050 with intermediate climate change levels)
rcp45.50.list=list.files(path="path to files", 
                         pattern="tif$", full.names=TRUE )
...
```{r eval=FALSE}
#change colors in map to match MaxEnt and RAMP
